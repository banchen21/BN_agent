...
use crate::ilink_sdk::error::{Error, HttpError, Result, SESSION_EXPIRED_ERRCODE};
use crate::ilink_sdk::http_client::HttpClient;
use crate::ilink_sdk::types::*;
...
impl ILinkClientBuilder<reqwest::Client> {
    ...
    pub fn build(self) -> ILinkClient<reqwest::Client> {
        let http = self
            .http
            .unwrap_or_else(crate::ilink_sdk::http_client::default_http_client);
        ILinkClient::from_parts(
            http,
            self.base_url,
            self.cdn_base_url,
            self.token,
    ...
    pub async fn login(
        self,
        handler: &dyn crate::ilink_sdk::auth::LoginHandler,
    ...
    pub async fn login_and_save(
        self,
        handler: &dyn crate::ilink_sdk::auth::LoginHandler,
        store: &crate::ilink_sdk::auth::CredentialStore,
    ) -> Result<ILinkClient<reqwest::Client>> {
        let http = self
            .http
            .unwrap_or_else(crate::ilink_sdk::http_client::default_http_client);
...
impl<H: HttpClient> ILinkClientBuilder<H> {
    ...
    pub fn build_with_http(self) -> Result<ILinkClient<H>> {
        let http = self.http.ok_or(Error::Other(
            "http_client is required for custom HttpClient".into(),
        ))?;
        Ok(ILinkClient::from_parts(
            http,
            self.base_url,
            self.cdn_base_url,
            self.token,
    ...
    pub async fn login_with_http(
        ...
        let http = self
            .http
            .ok_or(Error::Other("http_client is required".into()))?;
        do_login(
            http,
            self.base_url,
            self.cdn_base_url,
...
async fn do_login<H: HttpClient>(
    ...
    let result = crate::ilink_sdk::auth::QrLoginSession::start(&http, base, route_tag.as_deref())
        .await?
        .wait_for_login_with(handler)
        .await?;

    // Server may return a different base_url for this account.
    let final_base = result
        .base_url
        .as_ref()
        .filter(|u| !u.is_empty())
        .cloned()
...
impl<H: HttpClient> ILinkClient<H> {
    ...
    pub async fn push_text(&self, to: &str, text: &str) -> Result<String> {
        let ctx = self
            .get_context_token(to)
            .ok_or(Error::MissingContextToken)?;
        crate::ilink_sdk::messaging::send::send_text(self, to, text, &ctx).await
    }
    ...
    pub async fn send_text(&self, to: &str, text: &str, context_token: &str) -> Result<String> {
        crate::ilink_sdk::messaging::send::send_text(self, to, text, context_token).await
    }
    ...
    pub async fn send_image(
        ...
        crate::ilink_sdk::messaging::send::send_image(self, to, path, text, context_token).await
    }
    ...
    pub async fn send_video(
        ...
        crate::ilink_sdk::messaging::send::send_video(self, to, path, text, context_token).await
    }
    ...
    pub async fn send_file(
        ...
        crate::ilink_sdk::messaging::send::send_file(self, to, path, text, context_token).await
    }
    ...
    pub async fn send_media(
        ...
        crate::ilink_sdk::messaging::send::send_media(self, to, path, text, context_token).await
    }
    ...
    fn build_request(&self, endpoint: &str, body: Vec<u8>) -> http::Request<Vec<u8>> {
