...
use crate::ilink_sdk::cdn::aes_ecb::{decrypt_aes_ecb, parse_aes_key};
...
pub async fn download_and_decrypt_hex_key<H: HttpClient>(
    ...
    let aes_key_base64 = BASE64.encode(
        hex::decode(aeskey_hex).map_err(|e| Error::Aes(format!("invalid hex aeskey: {e}")))?,
    );
