use crate::ilink_sdk::cdn::download::{download_and_decrypt, download_plain};
use crate::ilink_sdk::client::ILinkClient;
use crate::ilink_sdk::error::Result;
use crate::ilink_sdk::http_client::HttpClient;
use crate::ilink_sdk::types::VoiceItem;
...
///
/// Only available when the `voice` feature is enabled.
///
/// ```ignore
/// use crate::ilink_sdk::voice::{download_voice, DefaultSilkDecoder};
...
pub async fn download_voice<H: HttpClient>(
    ...
    let media = voice
        .media
        .as_ref()
        .ok_or_else(|| crate::ilink_sdk::error::Error::Other("voice has no media".into()))?;
    let param = media.encrypt_query_param.as_deref().ok_or_else(|| {
        crate::ilink_sdk::error::Error::Other("voice has no encrypt_query_param".into())
    })?;
...
pub fn build_wav(pcm: &[u8], sample_rate: u32) -> Vec<u8> {
    assert!(
        pcm.len() <= u32::MAX as usize,
        "PCM data exceeds WAV 4GB limit"
    );
