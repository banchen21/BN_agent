...
use crate::ilink_sdk::client::{ILinkClient, is_api_error, is_session_expired};
use crate::ilink_sdk::error::Result;
use crate::ilink_sdk::http_client::HttpClient;
use crate::ilink_sdk::types::{GetUpdatesResponse, Message};
        ...
        loop {
            match &mut this.state {
                ...
                StreamState::Idle => {
                    ...
                    let fut: PollFuture =
                        Box::pin(async move { client.get_updates(&buf, timeout).await });
                    this.state = StreamState::Polling(fut);
                }

                StreamState::Polling(fut) => match fut.as_mut().poll(cx) {
                    ...
                    Poll::Ready(Ok(resp)) => {
                        if is_session_expired(&resp) {
                            ...
                            this.consecutive_failures = 0;
                            this.state = StreamState::Sleeping(Box::pin(tokio::time::sleep(
                        ...
                        let buf_updated = if let Some(new_buf) = &resp.get_updates_buf {
                ...
                StreamState::Sleeping(sleep) => match sleep.as_mut().poll(cx) {
                    Poll::Pending => return Poll::Pending,
                    Poll::Ready(()) => {
                        this.state = StreamState::Idle;
                    }
