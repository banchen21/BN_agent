...
impl Plugin for ClaudeBridgePlugin {
    ...
    fn start(&mut self, _ctx: PluginContext) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(ref reg) = _ctx.tool_registry {
            ...
            r.register(Arc::new(ClaudeSessionsTool {
                sessions: self.sessions.clone(),
            }));
            r.register(Arc::new(ClaudeClearSessionTool {
                sessions: self.sessions.clone(),
            }));
        }
...
impl ToolExecutor for ClaudeChatTool {
    ...
    fn execute(&self, args: &serde_json::Value) -> ToolResult {
        ...
        match call_claude_sync(&self.claude_path, &prompt, model.as_deref()) {
            Ok(r) => {
                if let Some(s) = &sid {
                    session_add_assistant(&self.sessions, s, &r);
                }
                ToolResult::ok(&r)
            }
...
struct ClaudeSessionsTool {
    sessions: Sessions,
}
impl ToolExecutor for ClaudeSessionsTool {
    fn def(&self) -> &ToolDef {
        static DEF: std::sync::LazyLock<ToolDef> = std::sync::LazyLock::new(|| ToolDef {
            name: "claude_sessions".into(),
            description: "List active Claude sessions.".into(),
            internal: false,
            parameters: serde_json::json!({}),
        });
        &DEF
    ...
    fn execute(&self, _: &serde_json::Value) -> ToolResult {
        ...
        if keys.is_empty() {
            ToolResult::ok("当前没有活动的 Claude 会话")
        } else {
            ToolResult::ok(&format!("活跃会话：\n{}", keys.join("\n")))
        }
    }
}

struct ClaudeClearSessionTool {
    sessions: Sessions,
}
impl ToolExecutor for ClaudeClearSessionTool {
    fn def(&self) -> &ToolDef {
        static DEF: std::sync::LazyLock<ToolDef> = std::sync::LazyLock::new(|| ToolDef {
            name: "claude_clear_session".into(),
            description: "Clear a session.".into(),
            internal: false,
            parameters: serde_json::json!({
