...
fn load_skills(dir: &str) -> Vec<SkillDef> {
    ...
    for entry in entries.flatten() {
        ...
        let (yaml_str, body) = match parse_skill_file(&content) {
            ...
            None => {
                log::info!(
                    "[skill] '{}' has no valid frontmatter (needs `---` delimiters), skipping",
                    path.display()
                );
                continue;
            }
    ...
    pub fn new() -> Self {
        Self {
            info: PluginInfo {
                ...
                version: "0.1.0".into(),
                description: "Skill Plugin — loads Markdown skills as callable tools for the LLM"
                    .into(),
                author: "BN Team".into(),
...
impl Plugin for SkillPlugin {
    ...
    fn start(&mut self, ctx: PluginContext) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if skills.is_empty() {
            log::info!(
                "[skill] WARN: no skills loaded ({} dir exists but no valid .md files)",
                skill_dir
            );
            return Ok(());
        }
        ...
        let registry = match ctx.tool_registry {
            ...
            None => {
                log::info!(
                    "[skill] WARN: no tool_registry available, skills will not be registered"
                );
                return Ok(());
            }
        };
