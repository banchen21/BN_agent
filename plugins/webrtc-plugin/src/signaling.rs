...
impl SignalingHandler {
    pub fn new(peers: Arc<Mutex<StdHashMap<String, peer::PeerHandle>>>) -> Self {
        Self { peers }
    }

    pub fn handle_event(&self, event: &Event) {
        if event.topic != "webrtc_signaling" {
            return;
        }
        ...
        let sig: SignalingMessage = match serde_json::from_value(
            ...
            Ok(s) => s,
            Err(e) => {
                log::warn!("[webrtc] signaling parse: {}", e);
                return;
            }
        };

        log::debug!(
            "[webrtc] signaling: peer={}, type={:?}",
            peer_id,
                ...
                match guard.get(&pid) {
                    Some(handle) => match sig {
                        ...
                        SignalingMessage::Offer { .. } => {
                            log::info!(
                                "[webrtc] Got Offer for peer {}. Use webrtc_answer_peer tool.",
                                pid
                            );
                        }
                    },
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
            ...
            let listener = match tokio::net::TcpListener::bind(&addr).await {
                Ok(l) => l,
                Err(e) => {
                    log::error!("[webrtc-signaling] bind {}: {}", addr, e);
                    return;
                }
            };
...
async fn handle_ws(
    ...
    log::info!(
        "[webrtc-signaling] new [{}]: room={}, role={}",
        peer_addr,
        room_id,
        role
    );
                    ...
                    tokio::spawn(async move {
                        ...
                        if let Some(handle) = guard.get(&pid) {
                            match sig_type.as_str() {
                                "Answer" => {
                                    if let Some(sdp) =
                                        sig_clone["signaling"]["payload"]["sdp"].as_str()
                                    {
                                        if let Err(e) = handle.set_remote_sdp(sdp).await {
                                            log::error!("[webrtc-signaling] set SDP: {}", e);
                                        } else {
                                            log::info!(
                                                "[webrtc-signaling] Answer applied: peer={}",
                                                pid
                                            );
                                        }
                                    }
                                ...
                                "IceCandidate" => {
                                    let c = sig_clone["signaling"]["payload"]["candidate"]
                                        .as_str()
                                        .unwrap_or("");
                                    let mid = sig_clone["signaling"]["payload"]["sdp_mid"]
                                        .as_str()
                                        .map(String::from);
                                    let idx = sig_clone["signaling"]["payload"]["sdp_mline_index"]
                                        .as_u64()
                                        .map(|v| v as u16);
                                    if !c.is_empty() {
                                        let _ =
                                            handle.add_ice_candidate(c, mid.as_deref(), idx).await;
                                    }
                                }
                                _ => {}
                            }
    ...
    fwd.abort();
    log::info!(
        "[webrtc-signaling] closed [{}]: room={}, role={}",
        peer_addr,
        room_id,
        role
    );
