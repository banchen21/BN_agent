...
impl BotHandle {
    async fn get_token(&self) -> Result<String, String> {
        ...
        if let Some(ref t) = *guard { return Ok(t.clone()); }
        ...
        let resp = self.client
            ...
            .send().await.map_err(|e| format!("token req: {}", e))?;

        let body: serde_json::Value = resp.json().await.map_err(|e| format!("token parse: {}", e))?;
        let code = body.get("code").and_then(|v| v.as_i64()).unwrap_or(-1);
        if code != 0 {
            return Err(format!("token error (code={}): {}", code,
                body.get("msg").and_then(|v| v.as_str()).unwrap_or("unknown")));
        }
        let token = body["tenant_access_token"].as_str().ok_or("no token")?.to_string();
    ...
    pub async fn send_message(&self, chat_id: &str, text: &str) -> Result<(), String> {
        ...
        let resp = self.client
            ...
            .send().await.map_err(|e| format!("send req: {}", e))?;
        let body: serde_json::Value = resp.json().await.map_err(|e| format!("send parse: {}", e))?;
        let code = body.get("code").and_then(|v| v.as_i64()).unwrap_or(-1);
        if code != 0 {
            Err(format!("send error (code={}): {}", code,
                body.get("msg").and_then(|v| v.as_str()).unwrap_or("unknown")))
        } else { Ok(()) }
    }
    ...
    pub async fn send_card(&self, chat_id: &str, card: &serde_json::Value) -> Result<(), String> {
        ...
        let resp = self.client
            ...
            .send().await.map_err(|e| format!("card req: {}", e))?;
        let body: serde_json::Value = resp.json().await.map_err(|e| format!("card parse: {}", e))?;
        let code = body.get("code").and_then(|v| v.as_i64()).unwrap_or(-1);
        if code != 0 {
            Err(format!("card error (code={}): {}", code,
                body.get("msg").and_then(|v| v.as_str()).unwrap_or("unknown")))
        } else { Ok(()) }
    }
}

async fn poll_messages(client: &reqwest::Client, token: &str) -> Result<Vec<serde_json::Value>, String> {
    let resp = client
    ...
    Ok(body.get("data").and_then(|d| d.get("items"))
...
fn process_message(msg: &serde_json::Value, eb: &Addr<EventBus>) {
    ...
    let chat_id = msg.get("chat_id").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let sender = msg.get("sender").and_then(|s| s.get("sender_id"))
        .and_then(|id| id.get("open_id")).and_then(|v| v.as_str())
        .unwrap_or("unknown").to_string();

    let content = msg.get("body").and_then(|b| b.get("content"))
        .and_then(|v| v.as_str()).unwrap_or("");
    let text = serde_json::from_str::<serde_json::Value>(content)
        .ok().and_then(|j| j.get("text").and_then(|v| v.as_str().map(String::from)))
        .unwrap_or_else(|| content.to_string());

    if text.is_empty() { return; }
    log::info!("[feishu-im] msg from {}: {}", sender, &text[..text.len().min(60)]);

    eb.do_send(Event::new(
        "user.message",
        serde_json::json!({"platform": "feishu", "chat_id": chat_id, "user_name": sender, "text": format!("[飞书]: {}", text)}),
        "feishu-im-plugin",
    ));
}

pub async fn run_bot(app_id: &str, app_secret: &str, eb: Addr<EventBus>) -> Result<BotHandle, String> {
    ...
    handle.get_token().await.map_err(|e| format!("auth failed: {}", e))?;
    ...
    tokio::spawn(async move {
        ...
        loop {
            match poll_handle.get_token().await {
                Ok(token) => {
                    match poll_messages(&poll_handle.client, &token).await {
                        Ok(msgs) => { for m in msgs { process_message(&m, &eb); } }
                        Err(e) => { log::debug!("[feishu-im] poll: {}", e); *poll_handle.token.lock().await = None; }
                    }
                }
