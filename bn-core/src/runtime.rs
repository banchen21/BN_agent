...
pub fn run() -> std::io::Result<()> {
    ...
    system.block_on(async {
        ...
        register_user_message_handler(
            &event_bus, llm_addr.clone(), plugin_manager.clone(),
            tool_registry.clone(), emitter, snapshots_for_cb.clone(),
        ).await;

        broadcast_startup(&event_bus, &plugin_manager).await;
        tracing::info!("BN Agent 运行中，按 Ctrl+C 退出...");

        start_api_server(plugin_manager.clone(), llm_addr, tool_registry.clone(), snapshots_for_cb);

        tokio::signal::ctrl_c().await.ok();
        tracing::info!("收到退出信号");

        plugin_manager.send(StopAll).await.ok();
        drop(plugin_manager); drop(event_bus); drop(tool_registry);
        tracing::info!("BN Agent 退出");
        actix_rt::System::current().stop();
    });

    if started { let _ = system.run(); }
    Ok(())
}
    ...
    match LlmConfig::from_env() {
        Ok(config) => {
            ...
            match LlmActor::new(config) {
                ...
                Err(e) => { tracing::error!("LLM Actor 创建失败: {}", e); None }
            }
        }
        Err(e) => { tracing::warn!("LLM 未配置: {}", e); None }
    }
}

async fn register_user_message_handler(
    event_bus: &Addr<EventBus>, llm_addr: Option<Addr<LlmActor>>,
    pm: Addr<PluginManager>, tool_registry: Arc<Mutex<ToolRegistry>>,
    emitter: Arc<BusEmitter>, snapshots_for_cb: Arc<Mutex<Vec<String>>>,
) {
    let llm_for_cb = llm_addr.clone(); let pm_for_cb = pm.clone();
    let tool_for_cb = tool_registry.clone(); let emitter_for_cb = emitter.clone();
    event_bus.send(RegisterCallback(Arc::new(move |event: &AgentEvent| -> bool {
        match event.event_type {
            EventType::UserMessage => {
                ...
                if let Some(ref llm) = llm_for_cb {
                    let llm = llm.clone(); let emitter = emitter_for_cb.clone();
                    let pm = pm_for_cb.clone(); let tool_registry = tool_for_cb.clone();
                    ...
                    actix::spawn(async move {
                        ...
                        core_loop::handle_user_message(&text, chat_id, &source, &llm, &emitter, &pm, &tool_registry, &snapshots, contexts).await;
                    });
                } else {
                    let reply = AgentEvent::new(EventType::AssistantMessage, EventSource::System, serde_json::json!({
                        "chat_id": chat_id, "text": format!("你说: {}", text), "source": source,
                    }));
                    emitter_for_cb.emit(reply.clone()); let _ = pm_for_cb.send(BroadcastEvent(reply));
                }
            }
...
async fn broadcast_startup(event_bus: &Addr<EventBus>, pm: &Addr<PluginManager>) {
    event_bus.send(EmitEvent(AgentEvent::new(EventType::SystemEvent, EventSource::System,
        serde_json::json!({"message": "系统启动完成"})))).await.ok();
    pm.send(BroadcastEvent(AgentEvent::new(EventType::PluginNotification, EventSource::System,
        serde_json::json!({"message": "宿主已就绪"})))).await.ok();
}

fn start_api_server(pm: Addr<PluginManager>, llm_addr: Option<Addr<LlmActor>>, tool_registry: Arc<Mutex<ToolRegistry>>, snapshots: Arc<Mutex<Vec<String>>>) {
    let api_llm = llm_addr.unwrap_or_else(|| panic!("LLM 未初始化"));
    actix_rt::spawn(async move {
        if let Err(e) = api_server::start_server(pm, api_llm, tool_registry, snapshots).await {
